// src/components/features/profile/AccountSettingsModal.jsx
import { useState } from 'react';
import { createPortal } from 'react-dom';

export default function AccountSettingsModal({ isOpen, onClose, user, onProfileUpdate }) {
  ...
  // Profile form state
  const [fullName, setFullName] = useState(user?.name || 'N/A');
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (activeTab === 'profile') {
        // --- JALUR API: Update Profil ---
        // TODO: Ganti mock dengan: await fetch('/api/profile', { method: 'PUT', body: { fullName, avatar } })
        // Atau Supabase: await supabase.from('profiles').update({ full_name: fullName, avatar_url }).eq('id', userId)
        await mockDelay(800);

        // Update sidebar profile secara dinamis
        if (onProfileUpdate) {
          onProfileUpdate({ name: fullName, avatar: avatarPreview, email: user?.email });
        }

      } else if (activeTab === 'security') {
        // --- JALUR API: Update Password ---
        // Validasi frontend
        if (!currentPassword) { setPasswordError('Kata sandi saat ini wajib diisi.'); setIsSaving(false); return; }
        if (!newPassword) { setPasswordError('Kata sandi baru wajib diisi.'); setIsSaving(false); return; }
        if (newPassword.length < 6) { setPasswordError('Kata sandi baru minimal 6 karakter.'); setIsSaving(false); return; }
        if (newPassword !== confirmPassword) { setPasswordError('Konfirmasi kata sandi tidak cocok.'); setIsSaving(false); return; }

        // TODO: Ganti mock dengan: await fetch('/api/auth/change-password', { method: 'POST', body: { currentPassword, newPassword } })
        // Atau Supabase: await supabase.auth.updateUser({ password: newPassword })
        await mockDelay(800);
  ...
  const modal = (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-xl max-w-2xl w-full mx-4 max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-bold text-slate-800 mb-4">Profil Publik</h3>
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <label className="bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-lg text-xs font-semibold hover:bg-gray-50 transition-colors cursor-pointer inline-block">
                        Ubah Foto
            ...
            {activeTab === 'security' && (
              <div className="space-y-6">
                ...
                {/* Current Password */}
                <div>
                  <label className="text-xs font-semibold text-slate-700 block mb-1.5">Kata Sandi Saat Ini</label>
                ...
                {/* New Password */}
                <div>
                  <label className="text-xs font-semibold text-slate-700 block mb-1.5">Kata Sandi Baru</label>
                  <div className="relative">
                    <input
                ...
                {/* Confirm Password */}
                <div>
                  <label className="text-xs font-semibold text-slate-700 block mb-1.5">Konfirmasi Kata Sandi Baru</label>
                  <div className="relative">
                    <input
                ...
                {/* Password error */}
                {passwordError && (
                  <p className="text-xs text-red-500 font-medium bg-red-50 px-3 py-2 rounded-lg">{passwordError}</p>
                )}
              </div>
            )}
            ...
            {activeTab === 'preferences' && (
              <div className="space-y-6">
                <h3 className="text-sm font-bold text-slate-800 mb-4">Preferensi Tampilan</h3>

                <div>
                  <label className="text-xs font-semibold text-slate-700 block mb-3">Tema Aplikasi</label>
                  <div className="flex gap-3">
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[10px] text-gray-400">Tema terang default</p>
                      </div>
                    </button>
                    <button
                    ...
                    >
                      <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-300"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
                      </div>
                      <div className="text-left">
                        <p className="text-sm font-semibold text-gray-800">Dark</p>
                        <p className="text-[10px] text-gray-400">Tema gelap (segera hadir)</p>
                      </div>
                    </button>
                  </div>
                </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between shrink-0">
          ...
          <div className="flex gap-3">
            <button onClick={onClose} className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-semibold transition-colors">Batal</button>
            <button onClick={handleSave} disabled={isSaving} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg text-sm font-semibold transition-colors disabled:opacity-70 flex items-center gap-2">
              {isSaving ? (
                <>
...
function UserIcon({ className = '' }) {
  return (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>);
}
function ShieldIcon({ className = '' }) {
  return (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>);
}
...
function EyeIcon({ show }) {
  if (show) return (<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>);
